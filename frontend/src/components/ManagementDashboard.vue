    ...
    <v-navigation-drawer permanent width="300" class="glass-drawer">
      <v-toolbar flat color="transparent" title="Engines & Logic">
        <template v-slot:append>
          <v-btn icon="mdi-plus-circle" color="primary" variant="text" @click="initiateAddProvider"
            title="Add Provider"></v-btn>
        </template>
      </v-toolbar>
      ...
      <v-list nav v-if="!isLoadingProviders">
        ...
        <v-list-item v-for="p in providers" :key="p.name" :value="p" :active="selectedProvider?.name === p.name"
          @click="selectProvider(p)" prepend-icon="mdi-brain" rounded="lg" class="mb-1">
        ...
        <v-window v-model="activeTab">
          ...
          <v-window-item :value="0">
            <v-row v-if="selectedProvider">
                ...
                <v-card class="glass-card pa-2" elevation="8">
                  ...
                  <v-card-text>
                    <v-form @submit.prevent="handleSaveProvider">
                      <v-row>
                        <v-col cols="12" md="6">
                          <v-text-field v-model="selectedProvider.name" label="Provider ID" placeholder="e.g. anthropic"
                            variant="outlined" density="comfortable"
                            :disabled="!!providers.find(p => p.name === selectedProvider?.name && selectedProvider?.name !== '')"
                            persistent-hint hint="The unique identifier for this provider"></v-text-field>
                        </v-col>
                        <v-col cols="12" md="6">
                          <v-text-field v-model="selectedProvider.api_key_name" label="Key Store Mapping"
                            placeholder="e.g. anthropic_api_key" variant="outlined" density="comfortable"
                            persistent-hint hint="Key name in internal storage"></v-text-field>
                        </v-col>
                        <v-col cols="12">
                          <v-text-field v-model="selectedProvider.api_key_value" label="Secret API Key"
                            placeholder="sk-..." variant="outlined" density="comfortable" type="password"
                            prepend-inner-icon="mdi-key" persistent-hint
                            hint="Managed directly for this provider"></v-text-field>
                        </v-col>
                        <v-col cols="12">
                          <v-text-field v-model="selectedProvider.base_url" label="Endpoint URL"
                            placeholder="https://api.example.com/v1" variant="outlined" density="comfortable"
                      ...
                      <v-row dense>
                        <v-col cols="6" sm="4">
                          <v-checkbox v-model="selectedProvider.openai_compatible" label="OpenAI Proto" color="primary"
                            hide-details density="compact"></v-checkbox>
                        </v-col>
                        <v-col cols="6" sm="4">
                          <v-checkbox v-model="selectedProvider.requires_api_key" label="Auth Required" color="primary"
                            hide-details density="compact"></v-checkbox>
                        </v-col>
                        <v-col cols="6" sm="4">
                          <v-checkbox v-model="selectedProvider.supports_vision" label="Vision (OCR)" color="primary"
                            hide-details density="compact"></v-checkbox>
                        </v-col>
                        <v-col cols="6" sm="4">
                          <v-checkbox v-model="selectedProvider.supports_streaming" label="Streaming" color="primary"
                            hide-details density="compact"></v-checkbox>
                        </v-col>
                        <v-col cols="6" sm="4">
                          <v-checkbox v-model="selectedProvider.supports_function_calling" label="Func Calls"
                            color="primary" hide-details density="compact"></v-checkbox>
                        </v-col>
                        <v-col cols="6" sm="4">
                          <v-checkbox v-model="selectedProvider.supports_tool_use" label="Tool Use" color="primary"
                            hide-details density="compact"></v-checkbox>
                        </v-col>
                      </v-row>

                      <v-select v-model="selectedProvider.modalities" label="Supported Modalities"
                        :items="modelModalities" multiple chips variant="outlined" class="mt-6"
                        closable-chips></v-select>
                    </v-form>
                  </v-card-text>
                  <v-card-actions class="pa-4">
                    <v-btn color="error" variant="tonal" prepend-icon="mdi-trash-can-outline"
                      @click="handleDeleteProvider(selectedProvider.name)">Delete</v-btn>
                    <v-spacer></v-spacer>
                    <v-btn color="primary" variant="elevated" size="large" prepend-icon="mdi-content-save-check"
                      @click="handleSaveProvider">Commit Configuration</v-btn>
                  </v-card-actions>
                </v-card>
              </v-col>
              ...
              <v-col cols="12" lg="5">
                <v-card class="glass-card" elevation="8">
                  ...
                  <v-divider></v-divider>
                  <v-data-table :headers="modelHeaders" :items="providerModels" :loading="isLoadingModels"
                    class="elevation-0 transparent-table" density="comfortable" height="300" fixed-header>
                    <template v-slot:item.modality="{ item }">
                      <v-chip size="x-small" :color="item.modality === 'llm' ? 'blue-darken-2' : 'purple-darken-2'"
                        variant="flat" label>
                        {{ item.modality.toUpperCase() }}
                      </v-chip>
                    </template>
                    <template v-slot:item.actions="{ item }">
                      <v-btn icon="mdi-close-circle-outline" size="small" color="red" variant="text"
                        @click="handleRemoveModel(item.value)"></v-btn>
                    </template>
                  </v-data-table>
                  ...
                  <v-card-text class="bg-surface-light">
                    ...
                    <v-row dense>
                      <v-col cols="12">
                        <v-text-field v-model="newModel.label" label="Friendly Label" placeholder="e.g. GPT-4o"
                          variant="outlined" density="compact" hide-details class="mb-2"></v-text-field>
                      </v-col>
                      <v-col cols="7">
                        <v-text-field v-model="newModel.value" label="Engine ID" placeholder="gpt-4o" variant="outlined"
                          density="compact" hide-details></v-text-field>
                      </v-col>
                      <v-col cols="5">
                        <v-select v-model="newModel.modality" :items="modelModalities" label="Type" variant="outlined"
                          density="compact" hide-details></v-select>
                      </v-col>
                    </v-row>
                    <v-btn block color="secondary" variant="flat" @click="handleAddModel" class="mt-4"
                      prepend-icon="mdi-plus">Register Model</v-btn>
                  </v-card-text>
                </v-card>
              </v-col>
            </v-row>
            <div v-else class="d-flex flex-column align-center justify-center py-16 text-grey-darken-2">
              ...
              <v-btn prepend-icon="mdi-plus-box" color="primary" variant="outlined" size="large" class="mt-8"
                @click="initiateAddProvider">Register New Provider</v-btn>
            </div>
          </v-window-item>

          <!-- System Prompts -->
          <v-window-item :value="1">
            <v-alert v-if="isLoadingPrompts" type="info" variant="tonal" class="mb-4">Synchronizing system
              personas...</v-alert>
            <v-row v-else>
              <v-col v-for="mod in promptModalities" :key="mod" cols="12" md="6">
                <v-card class="glass-card overflow-hidden">
                  <v-toolbar density="compact" color="transparent" flat>
                    <v-chip class="ml-4" color="secondary" size="small" variant="flat">{{ mod.toUpperCase() }}
                      CORE</v-chip>
                    <v-spacer></v-spacer>
                    <v-btn icon="mdi-content-save" size="small" color="primary" @click="handleSavePrompt(mod)"></v-btn>
                  </v-toolbar>
                  <v-textarea v-model="prompts[mod]" rows="10" variant="plain" class="px-4 py-2 code-font" no-resize
                    placeholder="Define the behavior for this modality..." bg-color="transparent"></v-textarea>
                </v-card>
              </v-col>
            </v-row>
          </v-window-item>
          ...
          <v-window-item :value="2">
            <v-card class="glass-card">
              <v-data-table :headers="keyHeaders" :items="formattedKeys" class="transparent-table">
                <template v-slot:top>
                  <v-toolbar flat color="transparent">
                    <v-toolbar-title class="font-weight-bold">Master Key Chain</v-toolbar-title>
                    <v-spacer></v-spacer>
                    <v-btn color="primary" variant="elevated" prepend-icon="mdi-key-plus" @click="keyDialog = true">Add
                      Manual Key</v-btn>
                  </v-toolbar>
                </template>
                ...
                <template v-slot:item.actions="{ item }">
                  <v-btn icon="mdi-delete-sweep-outline" size="small" color="error" variant="text"
                    @click="handleDeleteGlobalKey(item.name)"></v-btn>
                </template>
              </v-data-table>
            </v-card>
            ...
            <v-dialog v-model="keyDialog" max-width="500">
              <v-card class="glass-card pa-4">
                ...
                <v-card-text>
                  <v-text-field v-model="newKey.name" label="Service Identifier" placeholder="e.g. replicate"
                    variant="outlined" class="mb-2"></v-text-field>
                  <v-text-field v-model="newKey.value" label="Secret Value" type="password" variant="outlined"

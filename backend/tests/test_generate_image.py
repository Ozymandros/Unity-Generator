...
def test_generate_image_success(
    ...
    mock_agent_manager.run_image.return_value = AgentResult(
        image="base64-data", provider="openai"
    )

    monkeypatch.setattr("app.main.agent_manager", mock_agent_manager)

    response = client.post(
        "/generate/image", json={"prompt": "A hero portrait", "provider": "openai"}
    )
...
def test_generate_image_uses_image_provider_preference(
    client: TestClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Test that provider falls back to preferred_image_provider."""
    mock_agent_manager = MagicMock()
    mock_agent_manager.run_image.return_value = AgentResult(
        image="data", provider="dalle"
    )

...
try:
    from semantic_kernel import Kernel as RealKernel
...
def create_kernel(settings: dict[str, Any]) -> Any:
    """
    ...
    """
    if Kernel is None:
        raise RuntimeError("semantic-kernel is not installed. Install dependencies before running.")
    ...
    try:
        ...
        # agents is now in backend/app/agents
        semantic_plugins_dir = repo_root / "backend" / "app" / "agents" / "plugins" / "semantic"
        ...
        LOGGER.warning(f"Could not register semantic functions or fallback skills: {e}")

...
from app.core.config import load_api_keys as backend_load_api_keys
from app.core.db import get_pref, init_db, set_pref
from app.services import sprite_service
from app.services.agent_manager import AgentManager
from app.services.finalize_store import LOGGER, JobStatus, finalize_store
from app.services.image_provider import IMAGE_KEY_MAP
from app.services.unity_project import create_unity_project, get_latest_project_path
...
# sys.path hack removed - use environment PYTHONPATH if needed
...
def generate_sprites(request: SpritesRequest) -> GenerationResponse:
    ...
    try:
        ...
        # Always call generate_sprite, even if provider is None (let service handle error)
        ...
        # Always convert AgentResult to dict for type safety
...
def _run_finalize_in_background(job_id: str, request: FinalizeProjectRequest) -> None:
    ...
    """
    from app.services.unity_orchestrator import run_finalize_job
    ...
    try:
        ...
        if not project_path_str:
            ...
            try:
                ...
                store.update_job(job_id, error=f"Scaffold failed: {exc}", status=JobStatus.FAILED)
                return
        else:
        ...
        LOGGER.exception("Finalize job %s crashed", job_id)
        store.update_job(job_id, status=JobStatus.FAILED, error=f"Internal error: {exc}")

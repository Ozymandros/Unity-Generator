...
class StabilityImageAdapter(BaseProviderAdapter):
    ...
    def _do_invoke(
        ...
        response.raise_for_status()
        data = response.json()
        return AgentResult(image=data.get("image"), provider="stability", raw=data, model=model)
...
class FluxImageAdapter(BaseProviderAdapter):
    ...
    def _do_invoke(
        ...
        response.raise_for_status()
        data = response.json()
        return AgentResult(image=data.get("output"), provider="flux", raw=data, model=version)

...
- `Assets/Sprites/*.png`
- `Assets/Audio/*.mp3`
- `Assets/UI/` (Unity UI prefab scripts and YAML, generated by the Unity UI panel)
...
- Add a new endpoint by creating a request model in `schemas/__init__.py` and a route in the appropriate router under `app/routers/` (e.g. `generation.py` for generation endpoints).
